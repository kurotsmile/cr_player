if (typeof window.CR_Player === "undefined") {
  class CR_Player {
    ...
    onCreate(themeDefault = "theme_basic_top") {
      this.audio_player = new Audio();
      if (localStorage.getItem("cr_player_theme") != null)
        this.theme = localStorage.getItem("cr_player_theme");
      else this.theme = themeDefault;
      ...
      if (this.isLinkLoaded("all.min.css") == false)
        this.addCss(
    ...
    add_song(url_mp3) {
      var obj_data = {
        mp3: url_mp3,
        name: "Song " + this.list_song.length,
        artist: "Carrot Player Music",
      };
      this.list_song.push(obj_data);
          ...
          didOpen: () => {
            $(cr_player.list_theme).each(function (index, th) {
              if (cr_player.theme == th)
                $("#dropdown_theme").append(
                  $("<option>", { value: th, text: th, selected: true })
                );
              else
                $("#dropdown_theme").append(
                  $("<option>", { value: th, text: th })
                );
            });
          },
        })
    ...
    prev_song() {
      this.index_play_cur--;
      if (this.index_play_cur < 0)
        this.index_play_cur = this.list_song.length - 1;
      this.play_by_index(this.index_play_cur);
    }

    show_playlist() {
      var html =
        '<center class="text-center w-100 d-block" style="width:100%"><table style="width:100%" class="table table-striped table-hover table-responsive fs-9 w-100 text-break"><tbody style="width:100%" id="box_list_song"></tbody></table></center>';
      swal.fire({
        title: "Playlist",
        html: html,
        iconColor: cr_player.color_hightlight,
        ...
        didOpen: () => {
          $.each(cr_player.list_song, function (index, s) {
            ...
            if (cr_player.index_play_cur == index) {
              html +=
                '<td style="color:' +
                cr_player.color_hightlight +
                '"><i class="fas fa-volume-up"></i></td>';
              html +=
                '<td style="color:' +
                cr_player.color_hightlight +
                ';width:80%">' +
    ...
    loop() {
      this.index_loop_cur++;
      if (this.index_loop_cur >= this.list_loop_id.length)
        this.index_loop_cur = 0;
